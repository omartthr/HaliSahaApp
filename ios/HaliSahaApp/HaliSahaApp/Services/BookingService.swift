...
final class BookingService: ObservableObject {
    ...
    func getAvailableTimeSlots(
        ...
        let closeHourInt = Int(hours.close.prefix(2)) ?? 23 // "23" -> 23
        ...
        for hour in openHourInt..<closeHourInt {
            ...
            let slot = TimeSlot(
                ...
                bookingId: isBooked ? existingBookings.first { $0.startHour <= hour && $0.endHour > hour }?.id : nil,
                price: price
            )
    ...
    func cancelBooking(bookingId: String, reason: String?) async throws {
        ...
        // Cache'i temizle
        clearTimeSlotCache(facilityId: booking.facilityId, pitchId: booking.pitchId, date: booking.date)
    }
    
    // MARK: - Process Payment (Simulation)
    @MainActor
    func processPayment(booking: Booking, paymentMethod: PaymentMethod) async throws -> PaymentResult {
    ...
    func loadMockBookings() -> [Booking] {
        ...
        self.userBookings = mockBookings
        return mockBookings
    }
}
